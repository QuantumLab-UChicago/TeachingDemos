...
class BlochSphere extends BABYLON.Mesh {
    constructor(name, scene, inclinationRadians, azimuthRadians) {
        ...
        this.quantumStateArrowColor = new BABYLON.Color3(1, 0, 1);
    ...
    setProbAmplitudes(probAmp0, probAmp1) {
		...
		var f0 =1
		if (math.abs(probAmp0)>1)
		{
			f0 =1/math.abs(probAmp0)
			console.log("f0: " + f0);
		}
        var inclRads = 2 * math.acos(math.abs(probAmp0)*f0);
        console.log("inclRads: " + inclRads);
       // if (isNaN(inclRads))
       // {
       // 	this.setInclinationRadians(0);
       // }
       // else 
        //{
    ...
    applyGate(gate) {
        ...
        var amp0_mag = math.abs(math.subset(newQuantumState, math.index(0, 0)))
		var amp1_mag = math.abs(math.subset(newQuantumState, math.index(1, 0)))
		...
		var d =10 //Number of decimal places to avoid rounding errors
		var rnum = Math.pow(10, d)

        //var probAmp0 = math.divide(math.round(math.multiply(math.subset(newQuantumState, math.index(0, 0)),rnum*f0)),rnum);
        //var probAmp1 = math.divide(math.round(math.multiply(math.subset(newQuantumState, math.index(1, 0)),rnum*f1)),rnum);
    ...
    setupSphere() {
		...
		var RKet = this.makeTextPlane("|R⟩", "black", 0.2);
        RKet.position = new BABYLON.Vector3(1.2, 0.0, 0);
        RKet.isPickable = false;
		
		var LKet = this.makeTextPlane("|L⟩", "black", 0.2);
        LKet.position = new BABYLON.Vector3(-1.2, 0.0, 0);
        LKet.isPickable = false;
    ...
    createQuantumStateArrow(){
        ...
        arrowMaterial.diffuseColor = new BABYLON.Color3(0.0, 0.0, 1);
        arrowMaterial.specularColor = new BABYLON.Color3(0.0, 0.0, 1);
		...
		arrowPointMaterial.alpha = 0.0;

        var arrow = BABYLON.MeshBuilder.CreateLines("qStatePoints", { points: [this.sphere.position] }, this.scene);
        arrow.isPickable = false;

        var arrowBase = BABYLON.MeshBuilder.CreateCylinder("arrowBase", { height: 0.95, diameter: 0.02 }, this.scene);
        ...
        quantumStateLineCap.position = new BABYLON.Vector3(0, 0.955, 0);
