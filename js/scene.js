...
function createScene(engine, canvas, config) {
    ...
    var camera = new BABYLON.ArcRotateCamera("camera1", -Math.PI / 2.5, Math.PI / 2.5, 5, new BABYLON.Vector3(0, 0, 0), scene);
    camera.inputs.attached.pointers.buttons = [0]; // Make it so that the sphere doesn't move!
	...
	var qubitStateDiracImage = new BABYLON.GUI.Image("but", "http://localhost:8888/view/grok-bloch-master/images/qubit-state-dirac.png");
	//https://github.com/JavaFXpert/grok-bloch/blob/master/images/qubit-state-dirac.png
