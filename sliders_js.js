...
class RangeSlider 
{
	...
	initRange(range) {
		...
		if (circular) {
			...
			this.output.addEventListener('keydown', (event) => {
				switch(event.key) {
					case 'ArrowLeft': case 'ArrowDown': event.preventDefault(); this.stepUpMod();  this.updateCircle(); break;
					case 'ArrowRight': case 'ArrowUp': event.preventDefault(); this.stepDownMod(); this.updateCircle(); break;
					default: break;
				}
			});
	...
	*/
	rotate(x, y) {
		/* Because y is measured from the top, this angle is -1*angle in real space using normal math
		   convetions measuring from x-axis, CCW is positive */
	...
	*/
	...
	stepUpMod()
	{
	let a = this.range.valueAsNumber + parseFloat(this.range.step)
	if (a>=(this.max))
		{
			this.range.value = this.min+parseFloat(this.range.step)
		}
	else
		{
			this.range.stepUp()
	...
	stepDownMod()
	{
	let a = this.range.valueAsNumber - this.range.step
	if (a<=this.min)
		{
			this.range.value = this.max-this.range.step
		}
	else
		{
			this.range.stepDown()
	...
	updateCircle(start) {
		...
		if (!angle) 
		 {
		 	if (this.range.valueAsNumber>=this.max*3/4)
		 	{
		 		angle = rad * this.range.valueAsNumber + this.settings.offset - 360 ;
		 	}
		 	else
		 	{
		...
		let phase = this.wrapper.style.getPropertyValue('--gradient-phasereg')
		...
		let oldtheta = this.wrapper.style.getPropertyValue('--old-theta')
		
		if (phase ==0 && oldtheta>90 && oldtheta/end2<0)
		{
			
			phase = 1
			this.wrapper.style.setProperty('--gradient-phasereg', `${phase}`);
		}
		
		else if (phase ==0 && oldtheta<-90 && oldtheta/end2<0)
		{
			phase = -1
			this.wrapper.style.setProperty('--gradient-phasereg', `${phase}`);
		...
		else if (phase ==1 && oldtheta>-90 && oldtheta/end2<0)
		{
		...
		let start2=90
		if (end2<=90 && end2>=0 && phase==0) 
		{
			this.wrapper.style.setProperty('--gradient-start', `${end}deg`);
			this.wrapper.style.setProperty('--gradient-end', `${start2}deg`);
			this.wrapper.style.setProperty('--gradient-end2', `${360}deg`);
			this.wrapper.style.setProperty('--gradient-end3', `${360}deg`);
			console.log('end2='+end2+'prevval'+this.range.value)
			if (this.range.value<300)
			 {this.wrapper.dataset.value = -1*(this.range.value)+90;}
			else
			 {this.wrapper.dataset.value = -1*(this.range.value)+450;}
			
		}
		else if (end2<=0 && end2>=-180 && phase==0)
		{
			
			this.wrapper.style.setProperty('--gradient-start', `${start2}deg`);
		...
		else if (end2>90 && end2<=180 && phase==0)
		{
			end = end + 360;
			this.wrapper.style.setProperty('--gradient-start', `${0}deg`);
			this.wrapper.style.setProperty('--gradient-end', `${90}deg`);
			this.wrapper.style.setProperty('--gradient-end2', `${end}deg`);
			this.wrapper.style.setProperty('--gradient-end3', `${360}deg`);
			if (this.range.value!=0)
			 {this.wrapper.dataset.value = (450-this.range.value);}
			else
			 {this.wrapper.dataset.value = 90;}

		}
		
		else if (end2>=-180 && end2<=0 && phase==1)
		...
		else if (end2>90 && end2<=180 && phase==-1)
		{
			end = end + 360;
			this.wrapper.style.setProperty('--gradient-start', `${start2}deg`);
			this.wrapper.style.setProperty('--gradient-end', `${end}deg`);
			...
			this.wrapper.style.setProperty('--gradient-end3', `${360}deg`);
			console.log('end2='+end2+'prevval'+this.range.value)
			if (this.range.value>10)
			{this.wrapper.dataset.value = -1*(this.range.value)+90;}
			else
			{this.wrapper.dataset.value = -1*(this.range.value)-270;}

		}
		
		else if (end2<=90 && end2>=0 && phase==-1)
		{
			this.wrapper.style.setProperty('--gradient-start', `${0}deg`);
			this.wrapper.style.setProperty('--gradient-end', `${end}deg`);
			this.wrapper.style.setProperty('--gradient-end2', `${90}deg`);
			this.wrapper.style.setProperty('--gradient-end3', `${360}deg`);
			console.log('end2='+end2+'prevval'+this.range.value)
			if (this.range.value<300)
			 {this.wrapper.dataset.value = -1*(this.range.value)-270;}
		...
		this.wrapper.style.setProperty('--old-theta', `${end2}`);
		
		if (this.wrapper.dataset.value>=0)
		{
			this.wrapper.style.setProperty('--circle-bgc', `#FFF`);
			this.wrapper.style.setProperty('--track-bgc', `#f2f2f2`);
			this.wrapper.style.setProperty('--track-fill', `#ff8080`);
		}
		...
		
		{
			this.wrapper.style.setProperty('--circle-bgc', `#FFF`);
			this.wrapper.style.setProperty('--track-bgc', `#f2f2f2`);
			this.wrapper.style.setProperty('--track-fill', `#87aade`);
...
/* Parse through HTML and turn every element with data-range into a RangeSlider */
...
{
	let v= document.getElementById("slider_1").parentElement.getAttribute("data-value")

	alert(v);

}

function checkSliderPhase()
{
	let v= getComputedStyle(document.getElementById("slider_1").parentElement).getPropertyValue("--gradient-phasereg")	
	alert(v);
...
{
	let v= getComputedStyle(document.getElementById("slider_1").parentElement).getPropertyValue("--value-string")	
	alert(v);
	alert(typeof v);
